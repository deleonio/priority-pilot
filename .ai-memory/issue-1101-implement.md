...
- `server/src/api.d.ts`: Pfad `/geo/position` + Operation `reportGeoPosition` (204/400, Body lat/lon) ergänzt (nach `/geo-config` bzw. nach `updateGeoConfig`).
...
- api.d.ts ist hand-gepflegt: Pfad- UND Operations-Eintrag nötig, sonst TypeScript-Fehler im Frontend.
