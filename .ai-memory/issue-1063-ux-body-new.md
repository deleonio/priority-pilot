...
<!-- KI-ANALYSE:START stand=2026-08-27T17:54:18Z -->
### Entscheidung (bindend, @deleonio 2026-08-27)
- **Option B:** Serien-Datenmodell wird um `address` erweitert (analog Task), inkl. Vererbung auf generierte Instanzen.
- **Nicht im TaskTree:** Aufgaben-Badge nur in der Erledigt-Liste (`CompletedTasksTable`); Serien-Badge in der Serienliste (`SeriesTab`).
- **Icon:** Font-Awesome-Globus (`fa-solid fa-globe`) statt 🌍-Emoji.

### Umsetzungskontext
- Betroffene Dateien:
  - `server/src/models/series.ts` — `address?: string | null` + Spaltendefinition (nullable, VARCHAR(255), `defaultValue: null`), analog `server/src/models/task.ts:37`.
  - `server/src/logics/migrate.ts` — neue `migrateSeriesAddress` (Vorbild `migrateTaskAddress`, ~Zeile 424): Bestands-DBs ohne die Spalte nachziehen, sonst `no such column`.
  - `server/src/index.ts` — Migration vor `sequelize.sync()` aufrufen, bei den übrigen Serien-Migrationen (~Zeile 157-164) einreihen.
  - `server/src/express/routes/series.ts` — `validateSeriesFields` (~Zeile 129): `address` validieren (String ≤ 255 oder null, wie `routes/tasks.ts:204-208`); `serializeSeries` (~Zeile 109): zurückgeben; Kaskade `applyToInstances` (~Zeilen 393-402): geändertes `address` auf offene Instanzen übernehmen.
  - `server/src/logics/series.ts` — `generateDueInstances` (~Zeilen 136-152): `address: series.address ?? null` an jede `Task.create` vererben (Snapshot-Semantik wie `autoDeleteAfterDeadline`).
  - `openapi.yml` — Schemata `Series` (~1771), `SeriesCreate` (~1829), `SeriesUpdate` (~1874): `address` (string, nullable, maxLength 255); `applyToInstances`-Beschreibung (~1912) um `address` erweitern.
  - Client/Server-Typen neu generieren: `pnpm --filter client generate` + `pnpm --filter server build:api`.
  - `frontend/src/components/TaskForm.tsx` — Serien-Modus: Adressfeld mit bestehender `useAddressSearch`-Integration (wie Task-Modus); `SeriesCreate` (~561-571) und `SeriesUpdate` (~541-550) um `address` ergänzen; `hasSeriesCascadeChange` (~Zeile 70) um `address` erweitern (#553-Bestätigungs-Modal).
  - `frontend/src/components/SeriesTab.tsx` — Globus-Badge in der `series-tree-row` (~Zeile 146, Styling-Vorbild `series-tree-badge--rhythm`), nur bei gesetzter `address`.
  - `frontend/src/components/CompletedTasksTable.tsx` — Globus-Badge je Zeile für Tasks mit `address` (z. B. Titel-Zelle ergänzen); `TaskTree` bleibt bewusst unberührt.
- Betroffene Komponenten: Series-Modell + /series-API, Serien-Instanz-Generierung, TaskForm (Serien-Modus), SeriesTab-Liste, Erledigt-Tabelle.
- Vorhandenes Muster: `address` an Tasks (`server/src/express/routes/tasks.ts:204-208`, Tests `tasks-address.test.ts`); Feld-Vererbung `autoDeleteAfterDeadline` (`server/src/logics/series.ts`); Rhythmus-Badge (`frontend/src/components/SeriesTab.tsx:146`); FA-Icons etabliert (`fa-solid fa-pen` in SeriesTab-Toolbar).
- Randbedingungen: Bestands-DBs brauchen die Nachzieh-Migration vor `sync()`; TaskTree bekommt kein Badge; #553-Kaskade/Modal konsistent (address kaskadierbar wie `description`); erledigte Instanzen bleiben von der Kaskade ausgenommen (#555); `TaskTable.tsx` ist ungenutzt und wird nicht angefasst.
- Erwartetes Ergebnis: Serie mit Ortsbezug zeigt das Globus-Badge in der Serienliste; erledigte Tasks mit `address` zeigen es in der Erledigt-Liste; generierte Serien-Instanzen erben die Adresse; Einträge ohne `address` zeigen kein Badge.
- Kein Split: ein durchgängiger Vertikal-Schnitt mit je Datei kleinen Diffs, in einem PR reviewbar (Vorbild #523 `autoDeleteAfterDeadline` lief gleichartig).

### Akzeptanzkriterien
- AK1: `POST /series` und `PATCH /series/:id` akzeptieren `address` (String ≤ 255 Zeichen oder null) und `GET /series` gibt es zurück; ohne Angabe gilt `address === null`.
- AK2: `generateDueInstances` vererbt `address` als Snapshot auf jede neu generierte Instanz; nachträgliche Template-Änderungen wirken nur auf künftige Instanzen.
- AK3: `PATCH /series/:id` mit `applyToInstances=true` übernimmt ein geändertes `address` auf alle offenen (nicht erledigten) Instanzen; erledigte bleiben unverändert.
- AK4: In der Serienliste (`SeriesTab`) zeigt jede Serie mit `address` ein Globus-Badge; Serien ohne `address` zeigen keins.
- AK5: In der Erledigt-Liste (`CompletedTasksTable`) zeigt jeder Task mit `address` ein Globus-Badge; Tasks ohne `address` zeigen keins; der TaskTree zeigt kein Badge.
- AK6: Mobile (375px): das Badge verursacht in beiden Listen keinen horizontalen Überlauf/Layout-Bruch.

### Testfälle
- AK1 → API-Test (`node:test`, `server/src/express/series.api.test.ts` erweitern oder `series-address.test.ts` neu, Vorbild `tasks-address.test.ts`): Roundtrip create/update/get, 255-Limit, 400 bei Zahl, null löscht.
- AK2 → Unit-Test (`node:test`, `server/src/logics/series.test.ts`): Serie mit address → generierte Instanz trägt sie; ohne → null.
- AK3 → API-Test (`server/src/express/series.cascade.test.ts` erweitern): address-Kaskade auf offene Instanzen, Done-Instanz ausgenommen.
- AK4/AK5 → E2e (`frontend/e2e/*.spec.ts`, neue Spec-Datei nach Stil `crud.spec.ts`): Serie/Task mit Adresse → Badge sichtbar; ohne → kein Badge; TaskTree ohne Badge.
- AK6 → E2e, 375px-Viewport: Bounding-Box-Prüfung (kein Überlauf durch das Badge; App-Shell clippt `overflow-x`, daher Box-Messung statt scrollWidth).

### Ampel
- Ampel: 🟢
- Begründung: Produktentscheidung liegt bindend vor (Option B, kein TaskTree, FA-Icon); alle Muster (address an Tasks, Vererbung, Kaskade, Badge-Styling) sind im Repo vorhanden und werden analog angewandt.
