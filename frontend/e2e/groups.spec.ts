...
test.describe('Settings-Tab „Gruppen“ (#1211)', () => {
	...
	test('Angelegtabelle: Gruppe erscheint mit Rolle und Mitgliederzahl (AK6)', async ({ page }) => {
		...
		await accordionTrigger(page, 'E2E Familie').click();
	...
	test('Bearbeiten: nur geänderte Beschreibung speichern hält den Namen (AK6)', async ({ page }) => {
		...
		await accordionTrigger(page, 'E2E Edit').click();
	...
	test('Löschen verlangt sequenzielle Bestätigung und entfernt die Gruppe (AK7)', async ({ page }) => {
		...
		await accordionTrigger(page, 'E2E Weg damit').click();
	...
	test('Gruppen-Tab bleibt bei 375px ohne horizontalen Überlauf bedienbar (AK8)', async ({ page }) => {
		...
		await expect(card).toBeVisible();
		await accordionTrigger(page, 'E2E Schmal').click();
	...
	test('Mitgliederzeile mit Rollen-Button bleibt bei 375px ohne horizontalen Überlauf (#1221 AK8)', async ({
		...
		await accordionTrigger(page, 'E2E Rolle Schmal').click();
...
test('Gruppenliste zeigt das hinterlegte Gruppenbild als Avatar neben dem Namen (#1225 AK4)', async ({ page }) => {
	...
	await accordionTrigger(page, 'E2E Bild').click();
	await card.getByRole('button', { name: 'Bearbeiten' }).click();
...
test('Gruppe ohne Bild zeigt Initialen statt leerer Fläche (#1225 AK4)', async ({ page }) => {
	...
	await accordionTrigger(page, 'E2E Initiales').click();
...
test('Gruppenzeile mit Bild bleibt bei 375px ohne horizontalen Überlauf (#1225 AK5)', async ({ page }) => {
	...
	await accordionTrigger(page, 'E2E Mobil Bild').click();
	await card.getByRole('button', { name: 'Bearbeiten' }).click();
	await page.getByRole('textbox', { name: 'Bildadresse' }).fill('https://example.com/gruppe.png');
	await page.getByRole('button', { name: 'Speichern', exact: true }).click();
	await waitForStableView(page, 'Gruppen');
