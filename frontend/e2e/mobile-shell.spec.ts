...
/**
 * E2E-Spec für die Mobile-Optimierung der App-Shell: Der Kopfbereich wird auf Handy-Breite einzeilig
 * (Logo + primäre Aktion + „⋮"-Menü), die Seitenränder folgen der Mobile-First-Kaskade, und alle
 * bisherigen Kopf-Aktionen bleiben erreichbar.
 *
...
/** WCAG 2.5.5 / KoliBri `--a11y-min-size`: kleinste zulässige Kantenlänge eines Touch-Targets. */
...
test.describe('Mobile-Shell — Kopfbereich und Seitenränder', () => {
	...
	test('375×812: Menü-Trigger erfüllt das 44px-Touch-Target', async ({ page }) => {
		await gotoApp(page, MOBILE);

		const trigger = page.getByRole('button', { name: 'Mein Konto' });
	...
	test('375×812: eine Aktion aus dem Menü wirkt (Einstellungen navigiert)', async ({ page }) => {
		await gotoApp(page, MOBILE);
