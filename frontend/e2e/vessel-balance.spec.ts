...
/**
 * E2E-Spec für das Balance-Gefäß der Startseite (Lebensbalance).
 *
 * Nagelt den Kernpunkt fest, den nur die echte Seite belegen kann: Das Gefäß steht als erstes
 * Widget des Dashboards, ist horizontal mittig und bleibt auf schmalen Viewports innerhalb der
 * Seite. Die Segmentierung je Säule und die Rechnung prüfen die Komponenten- und Unit-Tests.
 *
 ...
 */
test.describe('Dashboard — Gefäß der Lebensbalance', () => {
	...
	const openDashboard = async (page: Page): Promise<void> => {
		...
		await page.request.post('/api/v1/tasks', { data: { title: 'E2E Lebensbalance' } });
	...
	test('steht als erstes Widget und mittig in seiner Karte (375×812)', async ({ page }) => {
		...
		const vessel = page.getByTestId(/vessel-balance-(canvas|svg)/);
		await expect(vessel).toBeVisible();

		// Je Säule eine Zeile in der Legende — ohne gepflegte Säulen gäbe es die Karte gar nicht.
		expect(await page.getByTestId('vessel-balance-legend-row').count()).toBeGreaterThan(0);

		// Horizontal mittig: die Grafik-Mitte liegt auf der Mitte ihrer Karte (1 px Toleranz für
		// subpixelgenaues Layout).
		const vesselBox = await vessel.boundingBox();
		const cardBox = await page.locator('.dashboard-vessel').boundingBox();
		expect(vesselBox).not.toBeNull();
		expect(cardBox).not.toBeNull();
		const vesselCenter = vesselBox!.x + vesselBox!.width / 2;
		const cardCenter = cardBox!.x + cardBox!.width / 2;
		expect(Math.abs(vesselCenter - cardCenter)).toBeLessThanOrEqual(1);

		// Das Gefäß ist das erste Widget unter der Begrüßung — es steht über den Statuskacheln.
	...
	// Die Grafik rendert mit `overflow: visible`, das Atmen skaliert über die Viewbox hinaus und der
	...
	for (const width of [375, 320]) {
		test(`bleibt bei ${width} px innerhalb der Seitenbreite`, async ({ page }) => {
			...
			await expect(page.getByTestId(/vessel-balance-(canvas|svg)/)).toBeVisible();

			for (const selector of ['.dashboard-vessel', '.vessel-balance-stage', '.vessel-balance-legend']) {
