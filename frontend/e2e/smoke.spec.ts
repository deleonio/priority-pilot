...
/**
 * Smoke-Test gegen das **echte** Backend (#91). Anders als `snapshots.spec.ts` und `forms.spec.ts`
 * wird hier **nichts** via `page.route` gemockt: Playwright startet ein echtes Express-Backend mit
 * temporärer In-Memory-DB (`:memory:`, `DB_RESET=true`, `DB_SEED=false`, siehe `playwright.config.ts`),
 * der Vite-Proxy reicht die API-Requests durch.
 *
 ...
 * Anfangszustand. Er bildet das Fundament für die funktionalen CRUD-Specs (#B).
 */
