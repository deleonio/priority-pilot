...
`../logo/logo-with-name.vertical.png` und wird aktuell nicht in der App eingebunden (kein Header/Login
verwendet es bislang – das ist eine separate, hier bewusst nicht angefasste Baustelle).
...
- `icon-512x512.png`, `icon-192x192.png` – Haupt-Icons (PWA-Manifest, `purpose: any`), aus dem
  kartenfreien Crop skaliert, Motiv füllt den Rahmen.
- `icon-512x512-maskable.png`, `icon-192x192-maskable.png` – Maskable-Varianten (`purpose: maskable`):
  Inhalt auf ~70 % der Canvas verkleinert und mit dem Logo-Mattenfarbton aufgefüllt, damit die zentrale
...
`public/` (liegen dort, nicht hier, da Browser Favicons standardmäßig auf Root-Ebene suchen).
...
Zielgrößen. Unter Linux/CI steht `sips` nicht zur Verfügung – dort ersatzweise ImageMagick
(`convert`/`magick`) mit denselben Crop-/Zielgrößen verwenden.
