...
/**
 * Basis-Style der Koordinaten-Box (#1111) — einmal definiert statt in beiden Varianten
 * (mit/ohne Koordinaten) dupliziert. Fontgröße über den Token (#1260, Audit F3: vorher
 * hartcodiertes `0.875rem` statt `var(--pp-font-size-sm)`).
 */
const COORDS_BOX_BASE_STYLE = {
	...
	fontSize: 'var(--pp-font-size-sm, 0.875rem)',
} as const;
...
export const TaskForm = ({
	...
	return (
		<>
			...
			<div className="form-grid form-grid--sections">
				...
				{/* #1159: Sekundärgruppe — Termin (Deadline bzw. Startdatum/Rhythmus) und Ort
				    ...
				    zugeklappt (AK2) — der Trigger beschriftet die Sektion selbst, daher entfällt die
				...
				<section className="form-section form-section--secondary">
					<KolAccordion
						...
						_open={scheduleOpen}
						_on={{
							onClick: (_event, open) => setScheduleOpen(open === true),
						}}
					>
						<div className="accordion-body">
			...
			{/* #1159: Optional-Bereich — Säulen, Beschreibung, Checkliste ohne eigene Fläche; die
			    Kennzeichnung trägt die bestehenden „… (optional)"-Labels. #1260: als KolAccordion
			...
			<section className="form-section form-section--optional">
				<KolAccordion
					...
					_open={optionalOpen}
					_on={{
						onClick: (_event, open) => setOptionalOpen(open === true),
					}}
				>
					<div className="accordion-body">
