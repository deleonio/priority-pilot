...
describe('HelpPage – #1206: Kategorien-Aggregation und klickbare Links', () => {
	...
	const releases695and694 = [
		{
			...
			body: [
				...
				'### Other Changes\n\n- Speichern beschleunigt ([#1204](https://github.com/deleonio/balamentum/pull/1204))\n- docs(guide): sync user guide by @deleonio in https://github.com/deleonio/balamentum/pull/1403',
			].join('\n\n'),
		},
		{
			tag_name: 'v0.1.694',
	...
	it('AK1: Markdown-Links zu Fremdseiten werden zu <a href>, Repo-Verlinkungen im Changelog werden entfernt', async () => {
		...
		expect(
			changelog?.querySelector('a[href*="github.com/deleonio/balamentum"]'),
			'keine Repo-Links im Changelog',
		).toBeNull();
		expect(changelog?.textContent, 'nackte Repo-URL fällt als Text weg').not.toContain(
			'https://github.com/deleonio/balamentum',
		);
		expect(changelog?.textContent, 'auch die URL des früheren Repo-Namens fällt weg').not.toContain(
