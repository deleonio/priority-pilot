 ...
 *
 * **Warum eine eigene Setup-Datei und nicht ein Block in `vitest.setup.ts`:** ES-Importe werden vor
 * dem Modulrumpf ausgewertet. Sobald `vitest.setup.ts` ein Modul importiert, das beim Laden Web
 * Storage anfasst, liefe dieser Shim zu spät. Genau das passierte mit `src/i18n/config.ts`: der
 * `LanguageDetector` prüft die Verfügbarkeit von `localStorage` einmal und **merkt sich das Ergebnis
 * modulweit** (`hasLocalStorageSupport`). Unter Node 26 fiel diese Prüfung auf `false`, das
 * Zurückschreiben der Sprachwahl war für den gesamten Testlauf still abgeschaltet — lokal unter
 * Node 22 grün, in CI rot. Diese Datei steht deshalb in `vitest.config.ts` VOR `vitest.setup.ts`.
 */
