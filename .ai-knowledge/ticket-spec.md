...
Label-Kette: `ai:analysed` → `ai:needs-ux-ui` → UX → **`ai:needs-impl` (dieser Workflow)** →
Umsetzung → PR.
...
- Der Workflow setzt am Issue **`ai:needs-impl`** (und konsumiert
  ...
  - Label bei Bedarf vorher anlegen
    (`gh label create "ai:needs-impl" --color FBCA04 --description "Zur Umsetzung freigegeben (Rote Tests stehen)"`).
