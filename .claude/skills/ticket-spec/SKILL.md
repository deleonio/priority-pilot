...
- **Red verification before the commit:** run ONLY the affected scope — `pnpm --filter <pkg> test`, narrowed with `-- <pattern>` where useful — never a repo-wide `pnpm test`. The failure must match the expected red state: assertion failure for changes to existing code, missing module only for genuinely new functionality; any other import/syntax error means the test is broken, not red — fix it before committing. Test runs are raw output → delegate them to the `gate-runner` role (ADR 0008, agent in `.claude/agents/`); you keep only command, exit code, and failing assertion.
