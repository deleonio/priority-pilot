...
- **Branch:** in **spec mode** the branch is already checked out. In **fallback mode**, the
  harness branch `ai/harness/<nr>` (exists only after an earlier spec/implementation run —
  resume case; the phase notes travel as workflow artifacts, ADR 0010, NOT on this branch;
  `git fetch origin && git switch ai/harness/<nr>`; otherwise `git switch -c ai/harness/<nr>`).
