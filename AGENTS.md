...
bzw. `ai:needs-spec` (Nicht-UI) → UX → `ai:needs-spec` → Spec → `ai:needs-impl` → Umsetzung → PR `ai:needs-review` → Review ↔ Fixup
(`ai:reviewed` + `ai:needs-fixup` → `ai:needs-review`) → `ai:reviewed` → Gate-Merge.
...
| **Review**        | `ai:needs-review` (am PR)                     | [review-kreuzverhoer-Skill](.claude/skills/review-kreuzverhoer/SKILL.md)                               | Sammelkommentar + Ampel → `ai:reviewed` (+ `ai:needs-fixup` bei 🔴)                                        |
