...
2. **`refresh()`:** Der Button **„Standort ermitteln"** (KolButton, sekundär) stößt eine sofortige Positionsermittlung plus anschließendes Reverse Geocoding an. Ein erneuter Aufruf, während eine Ermittlung läuft, wird ignoriert (Re-Entrancy-Guard — schützt das Nominatim-Rate-Limit). Bei `enabled=false` wird kein Button gerendert.
...
- Bei aktivierter Standorterfassung erscheint zusätzlich zum Aktivierungsschalter genau ein Button „Standort ermitteln"
