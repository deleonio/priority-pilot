...
**4. Die installierte PWA öffnet die App, nie die Website.** Neue Installationen starten über `start_url: /app/`. Die Manifest-`id` bleibt `/`, damit bestehende Installationen dieselbe App bleiben. Ältere Installationen starten weiter auf `/`, weil iOS das Manifest nicht aktualisiert und Android nur verzögert. Sie landen auf der Website und öffnen die App über „App öffnen“. Ein automatischer Sprung per `display-mode: standalone` war vorgesehen, ist aber vorerst zurückgenommen: Er ließ die Website nur aufblitzen und konnte mit einem Server-Fallback, der unter `/app/…` die Website ausliefert, eine Redirect-Schleife bilden.
