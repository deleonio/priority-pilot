...
scoresRouter.get('/scores/by-pillar', async (_req: Request, res: Response<PillarScoreDto[] | ErrorDto>) => {
	try {
		// Eine Abfrage statt N+1: ScoreEntry → Task → Pillar verschachtelt laden (keine Schleifen-Queries).
		const entries = await ScoreEntry.findAll({ include: [{ model: Task, include: [Pillar] }] });
