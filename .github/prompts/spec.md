...
ABLAUF (STRIKT):
  ...
  4. ROTE Tests schreiben – abgeleitet aus dem Spec (nicht direkt aus den Akzeptanzkriterien). Jedes AK muss durch den Spec gedeckt sein. Testebene nach Typ: Backend-Logik/API → server/src/{logics,express}/*.test.ts (node:test); Frontend-Logik → frontend/src/lib/*.test.ts (Vitest); Feature/UI-Verhalten → frontend/e2e/*.spec.ts (Akzeptanz-e2e); reines Styling/Layout → keinen Test erzwingen, stattdessen im PR-Body visuell begründen.
