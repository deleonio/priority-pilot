...
ORDER:
  1. COST BASELINE FIRST: run `bash .github/scripts/costs-summary.sh` — it prints the KOSTEN-BASIS table (per-phase runs, tickets, $, turns total, avg turns) plus the loop rates Fixup÷Implement / Review÷Implement and Turns/Ticket ready-made (runs without turn data are excluded from the averages, see its footnote). Loops, NOT phase totals, are the lever (review ranks high because of re-review rounds, not prompt inefficiency).
