...
FRAME (guiding question): NET cost over the whole phase chain, not the initial read — minimize initial cost without correctness, completeness, precision, or clarity suffering, and keep concreteness wherever it pays off. A prompt is only ONE part of the initial context; duplication against the knowledge base counts toward the same total. Compression is only a win if the prompt stays equally actionable; tokens that make it concrete (target files, exact commands, boundaries, expected output, small examples) are the highest-leverage ones — a prompt vague enough to trigger one clarification or fixup round (scale with the KOSTEN-BASIS loop rates from step 1) costs far more than hundreds of saved initial tokens. Cutting padding: good. Cutting specifics: regression. Subscriptions bill per turn, not per token: weigh findings in turns FIRST, tokens/$ second.

REVIEW TARGET (read it all yourself):
  1. CI prompts: all files under .github/prompts/ — phase prompts (triage, ux, spec, implement, fixup, review, documenter), memory snippets (memory-read.md, memory-write.md), and the nightly helper prompts (adr-sync, arc42-sync, code-review-team, design-optimize, mentor, spec-sync, guide-sync, ux-team, prompt-audit).
...
REVIEW CRITERIA (per prompt):
  ...
  H. TURN ECONOMY — does the prompt cause avoidable turns, or does it fail to demand bundling where the flow allows it (AGENTS.md rule "Turns bündeln" — reference it, don't restate it)? Typical faults: a step-by-step choreography that reads as one turn per step, a check run (GATE, tests, linter) per single change instead of once at the end over all of them, re-reading what is already in context, a built-in confirmation question (the pipeline's needs-human path is NOT one). An existing bundling instruction is a GOOD state: cutting it is a regression, not a saving — same protection as concreteness in B. Findings go in the category Turn-Bündelung.

ORDER:
  1. COST BASELINE FIRST: run `node .github/scripts/audit-basis.ts` — it prints the KOSTEN-BASIS (per-phase runs, tickets, turns, tokens in/out, $ in ONE table) plus completeness-filtered loop rates Fixup÷Implement / Review÷Implement, the First-Pass-Grün rate and Median Turns/Ticket ready-made (runs without turn data are excluded from the averages, see its footnote; incomplete tickets — fixup legs, aborted — only in the footnote sums). Loops, NOT phase totals, are the lever (review ranks high because of re-review rounds, not prompt inefficiency). Use it to VERIFY both turn AND token claims: turns are the billing unit and minimization target, tokens/$ the secondary axis.
...
REPORT (MANDATORY): write via bash heredoc to /tmp/prompt-audit.md. The workflow posts this file verbatim as the body of the ci:prompt-audit issue, so WRITE THE REPORT IN GERMAN (headings and findings alike) — the readers are the German-speaking maintainers. Structure:
  ...
  ## KOSTEN-BASIS
  Paste the script output verbatim (table + total row + loop-rates line + footnote, if any) — this is the data your finding prioritization is based on.
  ## Gesamturteil
  ...
  ## Funde
  ALL findings, sorted descending by expected NET saving (ties: severity). NET saving = turns avoided or caused (the billing unit; one fixup loop = fixup + re-review avg turns per your KOSTEN-BASIS) plus the initial-token delta — findings may save AND add both. Per finding:
  `### Rang <n> — <Datei> — <Kategorie>` (Redundanz|Kürzung|Konkretisierung|Turn-Bündelung|Delegation|Widerspruch|Unklarheit|Fehler)
  short quote — problem — concrete suggestion — expected NET saving (in TURNS first, then tokens/$ via the KOSTEN-BASIS averages) — **Machbarkeit** (leicht/mittel/schwer) — **Aufwand** (geschätzte Stunden).
  ...
  Per option: Titel — betroffene Dateien — erwartete NET-Ersparnis (turns first, then tokens + $ based on KOSTEN-BASIS averages) — Aufwand — Umsetzungsschritte.
  ## Entscheidung
